...
Text::Text(std::string path_str, GLFWwindow *window)
    : path_str(path_str),
      shader_program((path_str + "/../shaders/text_vert.glsl").c_str(),
                     (path_str + "/../shaders/text_frag.glsl").c_str()) {
...
void Text::load_font(std::string font, unsigned int font_size) {
    shader_program.use();
    FT_Library ft;
    ...
    std::string font_name = path_str + "/../fonts/" + font;
    if (font_name.empty()) {
    ...
    if (FT_New_Face(ft, font_name.c_str(), 0, &face)) {
        ...
        // 0 in width scales it automatically to height
...
void Text::render(std::string text, float x, float y, float scale,
                  glm::vec3 color) {
    shader_program.use();
    shader_program.set_vec3("textColor", color);
    ...
    for (c = text.begin(); c != text.end(); c++) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, VBO);
        // glBufferSubData not glBufferData
...
void Text::render(std::string text, glm::vec2 pos, float scale,
                  glm::vec3 color) {
    return render(text, pos.x, pos.y, scale, color);
}
